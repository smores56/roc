...
impl std::fmt::Debug for ArenaSmallStringInterner<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let strings: Vec<'_, _> = self.iter().collect_in(self.buffer.bump());

        f.debug_struct("ArenaSmallStringInterner")
...
impl<'a> ArenaSmallStringInterner<'a> {
    pub fn new_in(arena: &'a Bump) -> Self {
        Self {
            buffer: Vec::new_in(arena),
            lengths: Vec::new_in(arena),
    ...
    pub fn with_capacity_in(capacity: usize, arena: &'a Bump) -> Self {
        Self {
            ...
            buffer: Vec::with_capacity_in(5 * capacity, arena),

            lengths: Vec::with_capacity_in(capacity, arena),
    ...
    pub unsafe fn from_parts_in(
        ...
        let lengths = Vec::from_raw_parts_in(
    ...
    pub fn insert_index_str(&mut self) -> usize {
        ...
        write!(self.buffer.as_mut_slice(), "{index}").unwrap();
    ...
    #[inline(always)]
    pub fn find_indices(&'a self, string: &'a str) -> impl Iterator<Item = usize> + 'a {
...
mod test {
    ...
    use super::ArenaSmallStringInterner;

    #[test]
    fn update_key() {
        let arena = Bump::default();
        let mut interner = ArenaSmallStringInterner::new_in(&arena);
