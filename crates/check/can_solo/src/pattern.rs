...
pub enum Pattern<'a> {
    ...
    RecordDestructure {
        whole_var: Variable,
        ext_var: Variable,
        destructs: &'a [Loc<RecordDestruct<'a>>],
    },
    TupleDestructure {
        whole_var: Variable,
        ext_var: Variable,
        destructs: &'a [Loc<TupleDestruct<'a>>],
    },
    List {
        list_var: Variable,
        elem_var: Variable,
...
impl<'a> Pattern<'a> {
    pub fn opt_var(&self) -> Option<Variable> {
        ...
        match self {
            ...
            NumLiteral { var, .. } | IntLiteral { var, .. } | FloatLiteral { var, .. } => {
                Some(*var)
            }
            StrLiteral(_) => None,
            SingleQuote { .. } => None,
    ...
    pub fn category(&self) -> PatternCategory {
        ...
        match self {
            ...
            | OpaqueNotInScope(..)
            | UnsupportedPattern(..)
            | MalformedPattern(..) => C::PatternDefault,
        }
    }
}

#[derive(Clone, Debug, PartialEq)]
pub struct ListPatterns<'a> {
...
pub struct TupleDestruct<'a> {
    ...
    pub typ: (Variable, Loc<Pattern<'a>>),
}

#[derive(Clone, Debug, PartialEq)]
pub enum DestructType<'a> {
    Required,
    Optional(Variable, Loc<Expr<'a>>),
    Guard(Variable, Loc<Pattern<'a>>),
}

#[allow(clippy::too_many_arguments)]
pub fn canonicalize_def_header_pattern<'a, 'b>(
    env: &mut Env<'a, 'b>,
...
#[allow(clippy::too_many_arguments)]
pub fn canonicalize_pattern<'a, 'b>(
    env: &mut Env<'a, 'b>,
    var_store: &mut VarStore,
    scope: &mut Scope,
    output: &mut Output,
...
#[allow(clippy::too_many_arguments)]
pub fn canonicalize_record_destructs<'a, 'b>(
    env: &mut Env<'a, 'b>,
    var_store: &mut VarStore,
    scope: &mut Scope,
    output: &mut Output,
    ...
    region: Region,
    permit_shadows: PermitShadows,
) -> (&'a [Loc<RecordDestruct<'a>>], Option<Pattern<'a>>) {
    use ast::Pattern::*;

    let mut destructs = Vec::with_capacity_in(patterns.len(), env.arena);
    let mut opt_erroneous = None;
    ...
    (destructs.into_bump_slice(), opt_erroneous)
}

/// When we detect an unsupported pattern type (e.g. 5 = 1 + 2 is unsupported because you can't
/// assign to Int patterns), report it to Env and return an UnsupportedPattern runtime error pattern.
fn unsupported_pattern<'a, 'b>(
    env: &mut Env<'a, 'b>,
    pattern_type: PatternType,
    region: Region,
) -> Pattern<'a> {
...
pub enum BindingsFromPattern<'a> {
    Empty,
    One(&'a Loc<Pattern<'a>>),
    Many(&'a [BindingsFromPatternWork<'a>]),
}

pub enum BindingsFromPatternWork<'a> {
    Pattern(&'a Loc<Pattern<'a>>),
    RecordDestruct(&'a Loc<RecordDestruct<'a>>),
    TupleDestruct(&'a Loc<TupleDestruct<'a>>),
}
...
impl<'a> BindingsFromPattern<'a> {
    ...
    where
        I: Iterator<Item = &'a Loc<Pattern<'a>>>,
    {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self {
            ...
            BindingsFromPattern::One(loc_pattern) => match &loc_pattern.value {
                ...
                _ => {
                    *self = Self::Many(&[BindingsFromPatternWork::Pattern(loc_pattern)]);
                    self.next()
                }
            },
...
fn flatten_str_literal<'a>(literal: &'a StrLiteral<'a>) -> Pattern<'a> {
    use ast::StrLiteral::*;
...
fn flatten_str_lines<'a>(lines: &'a [&'a [StrSegment<'a>]]) -> Pattern<'a> {
    use StrSegment::*;
