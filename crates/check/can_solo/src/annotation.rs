...
impl<'a> Annotation<'a> {
    pub fn add_to(
        &self,
        aliases: &mut ArenaVecMap<'a, Symbol, Alias<'a>>,
...
pub(crate) fn canonicalize_annotation<'a, 'b>(
    env: &mut Env<'a, 'b>,
    scope: &mut Scope,
    ...
    let pol = match annotation_for {
        ...
        AnnotationFor::Custom => CanPolarity::InOpaque,
        AnnotationFor::Builtin => CanPolarity::InBuiltin,
    };
...
enum CanPolarity {
    ...
    InOpaque,
    /// In a builtin alias; polarity should be disregarded for now.
    InBuiltin,
    Neg,
    Pos,
}
...
impl CanPolarity {
    fn set_neg(self) -> Self {
        match self {
            CanPolarity::InAlias | CanPolarity::InOpaque | CanPolarity::InBuiltin => self,
...
/// `U8`, and `Str`.
pub fn find_type_def_symbols<'a>(
    scope: &mut Scope,
    ...
    let mut result = Vec::new_in(arena);

    let mut stack = vec![in arena; initial_annotation];
                ...
                Some(alias) => {
                    ...
                    for _ in 0..alias.infer_ext_in_output_variables.len() {
                        ...
                        let typ = match pol {
                            ...
                            // TODO: determine for opaques and builtins
                            CanPolarity::InBuiltin => todo!(),
