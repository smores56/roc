...
use crate::problem::CompilerProblem;
...
use roc_module::symbol::{IdentIdsByModule, ModuleId, PQModuleName, Symbol};
...
pub struct Env<'a, 'b> {
    ...
    // TODO: how to represent this in #![no_std]
    pub module_path: &'a Path,

    // TODO: if we allow "dequalifying" builtins (e.g. import List exposing [empty])
    // then we'll need to search this even without (...without what?)
    pub builtin_idents: &'b IdentIdsByModule,
    ...
    /// A mapping of symbols reprenting imported files to the relative path that was imported.
    pub ingested_files: ArenaVecMap<'a, Symbol, IngestedFile<'a>>,
...
impl<'a, 'b> Env<'a, 'b> {
    ...
    pub fn new(
        ...
        src: &'a str,
        module_path: &'a Path,
        builtin_idents: &'b IdentIdsByModule,
    ) -> Env<'a, 'b> {
        Env {
            src,
            home,
